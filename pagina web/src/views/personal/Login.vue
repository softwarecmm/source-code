        ...
        <v-col cols="12" md="8">
            <v-card height="100%" flat class="ml-sm-10 mr-sm-10">
                ...
                <v-sheet height="15%" width="100%" class="d-flex align-center">
                    <v-card-title class="primary--text text-h5 font-weight-bold">Inicio de sesion para personal</v-card-title>
                </v-sheet>
...
-->
        ...
        <v-col md="4">
            <v-card flat height="100%" class="d-md-flex d-none justify-center align-center">
                    <img src="@/assets/personal.png" height="30%">
            </v-card>
        </v-col>
    </v-row>
</template>
  ...
  methods: {
   async onSubmit () {
    try {
        ...
        if(validate){
          const response = await axios.post(this.host+'personal/login', JSON.stringify(this.user));
          if(response.data){
            await this.$store.dispatch('doLogin',response.data)
            this.$router.push('/personal/ingresos')
          }else{
            this.alert=true
            setTimeout(() => {
              this.alert=false
